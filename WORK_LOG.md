...
### 対応済み: ストア素材の準備（製品版リリース準備 #9・punch list最終項目）

- アプリアイコンをアイソメトリック立方体+"S"のデザインに刷新（別途対応済み、
  `assets/icon/`参照）。
- `docs/store-listing.md`を新規作成: アプリ名・サブタイトル・簡単な説明・詳しい
  説明・キーワード・カテゴリ・プロモーションテキスト・サポートURL、および
  年齢レーティング質問票への回答方針（ユーザー生成コンテンツあり、成人向け
  フィルタによる軽度の性的コンテンツの可能性あり、暴力表現はIGDBデータ由来の
  間接的なものである旨など）をドラフトとしてまとめた。ストア側の質問票自体は
  アカウント固有の操作のため代行できず、方針のみ提供。
- dev環境に使い捨てのデモアカウントを作成し、igdb-proxy経由で実在の人気タイトル
  （エルデンリング・ペルソナ5 ザ・ロイヤル・ゼルダの伝説 TOTK・スターデュー
  バレー・Hades）を記録・評価・レビュー付きで登録。実機エミュレータで
  ホーム・マイログ・ゲーム詳細・まとめ・実績の5画面をスクリーンショットし、
  ユーザーに送付。
- 誤って過去にコミットされていた`window_dump.xml`（UI検証時の一時ファイル）を
  削除し、`.gitignore`に`window_dump.xml`・`window_screenshot.png`・
  `screenshots/`を追加して再発防止。

これで製品版リリース準備の punch list（項目1〜9）はすべて着手・完了した
（項目4のCodemagic側SUPABASE_VARIABLES切り替えのみ、リリースタイミングに
合わせて後日実施する方針でユーザーと合意済み）。

---

## 2026-08-14（続き6）

### 対応済み: 自動テストの導入（製品版リリース準備 #8）

これまでテストが0件（`test/widget_test.dart`はプレースホルダーのみ）だったため、
費用対効果を優先し、Supabase通信を伴わない純粋なロジック関数を対象にユニット
テストを追加した。UIウィジェットテスト・E2Eは対象外（画面崩れ等は引き続き
実機確認に頼る）。

- `test/core/utils/age_test.dart`: 成人判定（`isAdultBirthYearMonth`）。誕生月
  境界値（誕生月そのものはまだ非成人扱いにする安全側の仕様）を含めて検証。
- `test/features/summary/period_summary_test.dart`: 月間・年間・すべて期間の
  集計（`summarizePeriod`）。期間の境界値（月末23:59は含む、翌月0:00は除外）、
  平均評価が未評価分を除外すること、ジャンル別集計が`genreLabel`で日本語
  ラベルに変換されることなどを検証。
- `test/features/achievements/achievement_test.dart`: 実績判定
  （`evaluateAchievements`）。記録数マイルストーンの達成日時、対応ハード種類数の
  累積判定、コレクション関連集計、連続記録月数（年またぎ・同月複数記録を含む）
  を検証。
- `codemagic.yaml`に「Run unit tests」ステップを追加（`flutter pub get`の直後）。
  `ignore_failure`を付けていないため、テストが1件でも落ちればビルド・
  パブリッシュ工程全体が実行されずに失敗する。

検証: `flutter test`で22件全て成功、`flutter analyze`は既存の無関係な警告のみで
クリーン。当初ジャンル集計のテストで実際の値と期待値が食い違う失敗が1件あり
（`genreLabel`はIGDBの正式ジャンル名を日本語ラベルに変換するため、テストの
テストデータにテスト用の適当な文字列ではなく実際のIGDB値を使う必要が
あった）、原因を特定してテスト側を修正した上で全件成功を確認している。
